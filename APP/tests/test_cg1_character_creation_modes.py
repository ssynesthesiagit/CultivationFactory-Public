...
def test_adversarial_planner_authority_and_invalid_stage2_fail_closed(tmp_path):
    ...
    svc,_,db=make(tmp_path/'bad',bad); before=snapshot(db); run=svc.start('p',execution_mode='STANDARD_API',idempotency_key='bad-plan-123')
    assert run['blockers'][0]['code']=='CG1_PLANNER_AUTHORITY_FIELDS_FORBIDDEN' and snapshot(db)==before
    svc,_,db=make(tmp_path/'invalid',plan(valid=False)); before=snapshot(db); run=svc.start('p',execution_mode='STANDARD_API',idempotency_key='invalid-123')
...
def test_atomic_restore_all_forced_failure_points(tmp_path):
    ...
    for point in points:
        svc,_,db=make(tmp_path/point,plan(combat=True)); run=svc.start('p',execution_mode='STANDARD_API',idempotency_key='failure-'+point); before=snapshot(db)
...
def test_revision_bound_before_transport_and_provider_fallback(tmp_path):
    svc,p,db=make(tmp_path/'revision'); run=svc.start('p',execution_mode='STANDARD_API',idempotency_key='revision-123'); child=svc.revise(run['run_id'],owner_notes='Change Method')
    ...
    svc,p,db=make(tmp_path/'fallback',ready=False); run=svc.start('p',execution_mode='STANDARD_API',idempotency_key='fallback-123')
...
def test_preference_is_durable_per_server_principal_and_finalize_uses_server_principal(tmp_path):
    ...
    run=same_owner.start('p',execution_mode='STANDARD_API',idempotency_key='principal-123')
