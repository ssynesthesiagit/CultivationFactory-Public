...
from ai_provider.secrets import DeepSeekSecretStore, SecretStore
...
class AIProviderService:
        ...
        *,
        ...
        if not MODEL_PATTERN.fullmatch(model):
            raise FoundryError(
                ...
                "The DeepSeek model ID must be 1-100 portable identifier characters.",
            )
        ...
        if enabled and (not data_sharing_acknowledged or not actor):
            raise FoundryError(
                ...
                "Enabling the provider requires a named acknowledgement that Stage 1 prompt data will be sent to DeepSeek.",
            )
        if len(actor) > 200:
    ...
    def _provider_call(self, *, key: str, request_bytes: bytes, timeout_seconds: int) -> tuple[int, bytes]:
        headers = {
        ...
        try:
            ...
            raise FoundryError(
                ...
                "The DeepSeek request failed before a complete response was received.",
    ...
    def complete_json(self, *, prompt_text: str, system_message: str, purpose: str) -> dict[str, Any]:
        ...
        settings = self._settings()
        if not settings["enabled"]:
            raise FoundryError("AI_PROVIDER_DISABLED", "The optional DeepSeek provider is disabled.")
        ...
        if key is None:
            raise FoundryError("AI_PROVIDER_API_KEY_MISSING", "No DeepSeek API key is available.")
        request_payload = {
            "model": settings["model"],
            "messages": [
        ...
        try:
            ...
            raise FoundryError(
                ...
                "DeepSeek returned a response that cannot enter the local JSON validator.",
    ...
    def run_stage1(self, prompt_id: str, *, idempotency_key: str) -> dict[str, Any]:
        ...
        if http_status != 200:
            self._update_run(
                ...
                **response_common,
                ...
                error_message=f"DeepSeek returned HTTP {http_status}.",
            )
            raise FoundryError(
                "AI_PROVIDER_HTTP_ERROR",
                "DeepSeek rejected the request.",
        ...
        try:
            ...
            raise FoundryError(
                "AI_PROVIDER_RESPONSE_CONTRACT_INVALID",
                "DeepSeek returned a response that cannot enter the Stage 1 validator.",
